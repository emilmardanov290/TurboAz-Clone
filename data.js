...
document.addEventListener("DOMContentLoaded", () => {
    ...
    data.forEach(car => {
        ...
        card.innerHTML = `
            <img src="${car.images[0]}" alt="${car.brand} ${car.model}" class="rounded-xl w-full h-48 object-cover mb-4">
            <h2 class="font-bold text-lg mb-1">${car.brand} ${car.model} (${car.year})</h2>
            <p class="text-gray-500 mb-1">${car.banType}</p>
            <p class="font-semibold mb-1">${car.price} ${car.currency}</p>
            <p class="text-sm text-gray-400 mb-2">${car.odometer} ${car.odometerUnit}</p>
            <p class="text-sm text-gray-400">${car.city} | ${car.dates}</p>
            <div class="flex gap-2 mt-2">
                ${car.credit ? '<span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Credit</span>' : ''}
                ${car.barter ? '<span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">Barter</span>' : ''}
            </div>
        `;
